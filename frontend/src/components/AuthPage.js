  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === "register") {
        ...
        if (data.microsoftBrowser)
          toast.success("Logged in via Microsoft browser ✅");
      ...
      if (err.code && err.code.startsWith("auth/")) {
        switch (err.code) {
          case "auth/configuration-not-found":
            msg =
              "Auth not configured. Please enable Email/Password in Firebase Console.";
            break;
    ...
    return (
      <div style={S.page}>
        <div style={S.card}>
          ...
          <p style={S.sub}>
            Current time: <strong>{blockedInfo.currentIST}</strong>
          </p>
          <button
            style={S.btn}
            onClick={() => {
      ...
      <div style={S.page}>
        <div style={S.card}>
          ...
          <div style={{ textAlign: "center", marginTop: 12 }}>
            ...
            <button style={S.link} onClick={handleResendOtp}>
              Resend OTP
            </button>
          </div>
          <div style={{ textAlign: "center", marginTop: 8 }}>
            <button style={S.link} onClick={() => setOtpStep(false)}>
              ← Back to Login
            </button>
          </div>
        </div>
