...
export const detectKeywords = (content, watchedKeywords = REQUIRED_KEYWORDS) => {
  if (!content) return { matched: false, matchedKeywords: [] };
  const allKeywords = [...new Set([...REQUIRED_KEYWORDS, ...watchedKeywords.map(k => k.toLowerCase())])];
  const matched = [];
...
export const fireTweetNotification = (tweet, matchedKeywords = []) => {
  ...
  const body = tweet.content?.length > 180 ? tweet.content.slice(0, 177) + '...' : tweet.content;
  const n = new Notification(`🐦 TweetApp · ${tags}`, {
    body, icon: '/favicon.ico', tag: `tweet-${tweet._id}`, requireInteraction: false,
  });
...
export const maybeNotify = (tweet, prefs) => {
  ...
  const { matched, matchedKeywords } = detectKeywords(tweet.content, prefs.keywords || REQUIRED_KEYWORDS);
