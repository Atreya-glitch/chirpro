...
export default function DashboardPage() {
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      if (fired) {
        const { matchedKeywords } = detectKeywords(
          newTweet.content,
          user?.notificationPreferences?.keywords,
        );
        toast(
      ...
      toast.success(
        `Posted! ${rem !== "Unlimited" ? `${rem} remaining` : "Unlimited 🎉"}`,
      );
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${API_URL}/tweets/${id}`, {
        headers: getAuthHeaders(),
      });
      ...
      {limit !== Infinity && (
        <div style={{ paddingInline: 4 }}>
          ...
          >
            <div
              style={{
                height: "100%",
                width: `${prog}%`,
                background: prog >= 100 ? "#ef4444" : PLAN_COLORS[plan],
                borderRadius: 10,
          ...
          <div style={{ fontSize: 11, color: "#9ca3af", marginTop: 3 }}>
            {limit - count <= 0
              ? "Limit reached"
              : `${limit - count} tweet${limit - count !== 1 ? "s" : ""} left`}
          </div>
        </div>
      )}
      <div
        ...
        <div>
          <div
            style={{
              fontSize: 12,
              fontWeight: 700,
              color: notifOn && isGranted ? "#166534" : "#6b7280",
            }}
          >
          ...
          <div style={S.banner}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, marginTop: 2 }}>
                Get notified for <strong>#cricket</strong> and{" "}
                <strong>#science</strong> tweets.
              </div>
            </div>
            <button style={S.bannerBtn} onClick={requestPermission}>
              Enable Now
            </button>
          </div>
        )}
        ...
        <div style={S.composeBox}>
          ...
          <div style={S.cFooter}>
            <span
              style={{
                fontSize: 13,
                color: content.length > 260 ? "#ef4444" : "#9ca3af",
              }}
            >
            ...
            <button
              style={{
                ...S.postBtn,
                background: canPost ? "#1da1f2" : "#9ca3af",
          ...
          {!canPost && (
            <div style={S.limitWarn}>
              ...
              <span
                style={{ color: "#1da1f2", cursor: "pointer", fontWeight: 600 }}
          ...
          tweets.map((t) => {
            ...
            return (
              <div
                ...
                style={{
                  ...S.tweet,
                  borderColor: matched ? "#fde68a" : "#e5e7eb",
                }}
              >
                <div style={S.tweetAvatar}>
                  ...
                  >
                    ...
                    <span
                      style={{
                        ...
                        fontWeight: 600,
                        color:
                          PLAN_COLORS[t.user?.subscription?.plan || "free"],
                      }}
                    >
                      {PLAN_ICONS[t.user?.subscription?.plan || "free"]}{" "}
                      {t.user?.subscription?.plan || "free"}
                    </span>
                    {matched && (
